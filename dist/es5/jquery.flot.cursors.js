    ...
    function init(plot) {
        ...
        var thumbmove = {
            ...
            drag: function(e) {
                ...
                if (currentlySelectedCursor) {
                    currentlySelectedCursor.selected = false;
                    currentlySelectedCursor.thumbsmove = false;
                }
            }
        };
        ...
        plot.hooks.bindEvents.push(function (plot, eventHolder) {
            ...
            eventHolder[0].addEventListener('thumbsmovestart', thumbmove.start, false);
            eventHolder[0].addEventListener('thumbsmove', thumbmove.drag, false);
            eventHolder[0].addEventListener('thumbsmoveend', thumbmove.end, false);
        ...
        function handleThumbsMoveStart(e) {
            var currentlySelectedCursor = selectedCursor(cursors);
            if (!currentlySelectedCursor) {
                ...
                visibleCursors(cursors).forEach(function(cursor) {
                    ...
                    cursor.thumbs.forEach(function (thumb) {
                        if (thumb === e.detail.target) {
                            targetCursor = cursor;
                        }
                    });
                });
        ...
        function createThumbs(cursor, plot, plotOffset) {
            var svgRoot = $.thumb.createSVGLayer(plot.getPlaceholder(), plot.getEventHolder()),
                ...
                thumbCy = plot.height() + plotOffset.top + plotOffset.bottom - 15,
                horizontalConstraint = function (mouseX, mouseY, currentX, currentY) {
                    return [mouseX, currentY];
                };
            thumb = $.thumb.createThumb(12, thumbCx, thumbCy, svgRoot);
            thumb.constraintFunction = horizontalConstraint;
        ...
        plot.hooks.shutdown.push(function (plot, eventHolder) {
            ...
            eventHolder.unbind('thumbsmovestart', thumbmove.start);
            eventHolder.unbind('thumbsmove', thumbmove.drag);
            eventHolder.unbind('thumbsmoveend', thumbmove.end);
            plot.getPlaceholder().css('cursor', 'default');
        });
    }
...
$(function () {
    ...
    $.thumb.createThumb = function (radius, cx, cy, svgRoot) {
    ...
    function selectElement(evt) {
        ...
        dispatchThumbEvent('thumbsmovestart', evt);
    }
    ...
    function moveElement(evt) {
        ...
        dispatchThumbEvent('thumbsmove', evt);
    }
    ...
    function deselectElement(evt) {
        if (currentState) {
            ...
            mainEventHolder.dispatchEvent(new CustomEvent('thumbsmoveend', { detail: evt }));
        }
    }
